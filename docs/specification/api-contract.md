...
This document is the behavioral contract for api/. All implementations must follow this contract strictly.
...
No authentication in MVP. All endpoints are accessible locally only. Future phases may add API key or session-based auth.
...
Purpose: Download Metadata TXT files for an RFC and date range.
...
Required fields: rfc, cer_path, key_path, password, start_date, end_date Optional fields: tipo (default: recibidos), output_dir (default: ./results_RFC), intervalo (default: 60)
...
Purpose: Download CFDI XML files for an RFC and date range. Applies automatic datetime offset bypass. Returns immediately after SAT acceptance with request ID.
...
Required fields: rfc, cer_path, key_path, password, start_date, end_date Optional fields: tipo, output_dir, intervalo, timeout_min (default: 30)
...
Purpose: Resume polling for a specific pending CFDI request.
...
Purpose: Execute full end-to-end flow: Metadata income + Metadata expenses + Excel generation.
...
Required fields: rfc, cer_path, key_path, password, start_date, end_date Optional fields: output_dir, intervalo, despacho, regimen (default: resico)
...
Purpose: Generate Excel working paper from already downloaded Metadata TXT files.
...
Required fields: rfc, start_date, end_date, income_files, expense_files Optional fields: output_dir, despacho, regimen
...
Purpose: Generate Excel with exact tax breakdown from CFDI XML files. PLANNED — Phase 7.
...
Purpose: Return all pending requests across all RFCs.
...
Purpose: Return pending requests for a specific RFC.
...
Purpose: Return saved RFC profile.
...
Purpose: Return encrypted attempt history for an RFC.
