...
# ===========================================================================
# Descarga reusable - usada por run.py y --excel
# ===========================================================================
...
def descargar_cfdi(
    ...
    """
    Descarga CFDIs del SAT para un periodo y tipo especifico.
    Maneja el bypass de offset via cache encriptado automaticamente.

    Parametros:
      fiel       : objeto Fiel cargado
      rfc        : RFC del contribuyente
      inicio     : fecha inicio del periodo
      fin        : fecha fin del periodo
      tipo       : 'emitidos' o 'recibidos'
      output_dir : carpeta donde se guardaran los ZIPs y XMLs
      intervalo  : segundos entre verificaciones

    Retorna:
      Lista de rutas (Path) a los XMLs extraidos.
      Retorna lista vacia si no hay CFDIs o hubo error.
    """
    _validar_salt()

    params_local = {
        "rfc": rfc, "inicio": inicio, "fin": fin,
        "tipo": tipo, "intervalo": intervalo, "output": output_dir,
        "solicitud": "CFDI",
    }

    # Consultar y registrar historial para bypass de offset
    historial = consultar_historial(rfc, inicio, fin, tipo)
    offset_actual = registrar_intento(rfc, inicio, fin, tipo)
    dt_inicio, dt_fin = fechas_con_offset(inicio, fin, offset_actual)

    log.info(f"  Descargando {tipo} | Periodo efectivo: {dt_inicio} → {dt_fin}")

    token = obtener_token(fiel)
    id_solicitud = solicitar_descarga(fiel, token, params_local, dt_inicio, dt_fin)

    if not id_solicitud:
        log.warning(f"  ✗ SAT no acepto la solicitud de {tipo}. Sin CFDIs en el periodo.")
        return []

    paquetes = verificar_solicitud(fiel, id_solicitud, params_local)

    if not paquetes:
        log.warning(f"  Sin paquetes para {tipo}. Sin CFDIs en el periodo.")
        return []

    nombre_base = f"{inicio.strftime('%Y-%m')}-{rfc}"
    ...
    log.info(f"  ✔ {tipo}: {len(xmls_totales)} XML(s) descargados.")
    return xmls_totales


def _organizar_xmls_para_excel(
    xmls: list[Path],
    rfc: str,
    base_dir: Path,
    mes_key: str,
    ...
    """
    Clasifica XMLs descargados en carpetas ingresos/ y gastos/
    segun si el RFC es emisor (ingreso) o receptor (gasto).

    Retorna (ingresos_dir, gastos_dir).
    """
    from xml_parser import parsear_cfdi
    import shutil

    ingresos_dir = base_dir / "ingresos" / mes_key
    gastos_dir   = base_dir / "gastos"   / mes_key
    ...
    for xml_path in xmls:
        rec = parsear_cfdi(xml_path)
        if rec is None:
            continue

        if rec["rfc_emisor"].upper() == rfc.upper():
            dest = ingresos_dir / xml_path.name
            shutil.copy2(xml_path, dest)
            n_ingresos += 1
        elif rec["rfc_receptor"].upper() == rfc.upper():
            dest = gastos_dir / xml_path.name
            shutil.copy2(xml_path, dest)
            n_gastos += 1

    log.info(f"  XMLs organizados: {n_ingresos} ingresos, {n_gastos} gastos")
    return ingresos_dir, gastos_dir
...
def main() -> None:
    ...
    # -----------------------------------------------------------------------
        ...
        # --- Post-procesamiento --excel ---
        if params.get("excel") and xmls_totales:
            log.info("")
            log.info("=" * 65)
            log.info("GENERANDO EXCEL POST-DESCARGA")
            log.info("=" * 65)

            try:
                from excel_generator import generate_excel_from_xml
                from config import TABLA_ISR_RESICO_DEFAULT

                base_dir  = params["output"]
                mes_key   = params["inicio"].strftime("%Y-%m")

                # Organizar XMLs en ingresos/ y gastos/
                ingresos_dir, gastos_dir = _organizar_xmls_para_excel(
                    xmls_totales, params["rfc"], base_dir, mes_key
                )
